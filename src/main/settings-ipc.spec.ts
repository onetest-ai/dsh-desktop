  ...
  describe('plugins', () => {
    ...
    it("classifies a config-validation reason as needs-configuration, a non-validation reason as failed, and leaves a healthy entry's kind undefined", () => {
      const d = deps({
        readConfig: () => ({
          ...
          config: withPlugins([
            { spec: `${DECK}@0.2.1`, version: '0.2.1' },
            { spec: '@onetest/dsh-widget@1.0.0', version: '1.0.0' },
            { spec: HOOKS_PACKAGE, version: '0.1.1-rc.2' },
          ]),
        }),
        disabledPlugins: () => ({
          ...
          '@onetest/dsh-widget':
            'failed to apply loader entry onetest-dsh-widget (/home/dev/.dsh/plugins/onetest-dsh-widget/lib/index.js): invalid config: - expected { transport?: "stdio", serverName: string, command: string } but got {}',
        }),
      })
      ...
      expect(plugins.find((plugin) => plugin.package === '@onetest/dsh-widget')?.disabledKind).toBe(
        'needs-configuration',
      )
    ...
    it('drops the loader-entry preamble from a needs-configuration summary, keeping the expected shape', () => {
      const d = deps({
        readConfig: () => ({
          configured: true,
          config: withPlugins([{ spec: '@onetest/dsh-widget@1.0.0', version: '1.0.0' }]),
        }),
        disabledPlugins: () => ({
          '@onetest/dsh-widget':
            'failed to apply loader entry deepseek-ai-dsh-mcp-client (@onetest/dsh-widget): invalid config: - expected { transport?: "stdio", serverName: string, command: string } but got {}',
        }),
      })

      const { plugins } = createSettingsHandlers(d).read()
      const summary = plugins.find((plugin) => plugin.package === '@onetest/dsh-widget')?.disabledSummary
...
describe('save', () => {
  ...
  describe('plugins', () => {
    it('installs a pinned entry at exactly its spec\'s version, reusing the shared installer', async () => {
      ...
      // The bridge is prepended by `withHookBridge` (this is a non-managed
      // harness, so it lands bare) and now persists alongside the deck.
      expect(d.writeConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          plugins: [{ spec: HOOKS_PACKAGE, version: '0.2.1' }, { spec: `${DECK}@0.2.1`, version: '0.2.1' }],
        }),
      )
    })
    ...
    it('persists a row config alongside the resolved entry', async () => {
      ...
      expect(d.writeConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          plugins: [
            { spec: HOOKS_PACKAGE, version: '0.2.1' },
            { spec: `${DECK}@0.2.1`, version: '0.2.1', config: { base: '/x' } },
          ],
        }),
      )
    })
    ...
    it('adds and removes entries, round-tripping through config, bridge included', async () => {
      ...
      // a third entry. The bridge is a visible default now, so it persists
      // like any other entry — this is a non-managed (local) harness, so
      // `withHookBridge` has no harness version to re-pin to and leaves the
      // form's bare bridge entry as typed.
    ...
    it('persists the bridge re-pinned to the current harness version, even when a prior save had left a stale one on disk', async () => {
      // The bridge is a visible default now, so it is persisted like any
      // other plugin — but `withHookBridge` re-pins any existing bridge entry
      // to the managed harness's current version before install, so a stale
      // persisted pin (from an earlier harness version) can never survive a
      // save; it always comes back out at the version this save resolved.
      ...
      const bridge = written.plugins?.find((entry) => entry.spec.includes(HOOKS_PACKAGE))
      expect(bridge).toEqual({ spec: `${HOOKS_PACKAGE}@0.1.1-rc.2`, version: '0.1.1-rc.2' })
    })
