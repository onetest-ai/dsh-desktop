...
`
  ...
  if (ready.length === 0) return { overlay: `${webserver}${sidebarDisable}\n${comments}`, rows: [] }
  ...
  return {
    overlay: `${webserver}${sidebarDisable}
- insert:
${inserts}
${comments}`,
    rows,
...
export function writeRuntimeFiles(
  ...
  const built = patchOverlay(ready, omitted, hideBuiltinRightSidebar)
