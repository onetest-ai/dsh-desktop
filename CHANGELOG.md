...
- **The view tools are named for what they do, under two prefixes**: `editor_*` for the column beside the conversation (`editor_open_file`, `editor_show_diff`, `editor_selection`) and `browser_*` for the page. The three ways of getting at a page now say what each gives back — `browser_open` navigates and returns the text, `browser_read` returns the text of whatever is already showing, `browser_snapshot` numbers the controls — where `browse_page`, `read_open_page` and `browser_read_page` were near-twins a model guessed wrong.
...
- **A browser in the pane**, with back, forward, reload, and an address bar that takes a bare host — a real page in its own process, with no preload of its own. The agent reads through it (`browser_open`, `browser_read`) and drives it through the Chrome DevTools protocol: `browser_snapshot` numbers the page's controls, and `browser_click`, `browser_type`, `browser_press_key`, `browser_select_option`, `browser_drag`, `browser_upload_file`, `browser_handle_dialogs`, `browser_evaluate`, `browser_console`, `browser_resize`, and `browser_screenshot` act on them. Input goes through the protocol rather than through injected JavaScript, so the page receives it as the user's own — which is what native dialogs, file inputs, and drag-and-drop libraries require. Playwright's own MCP server drives a separate headless browser; every description here says which browser it means.
