...
- **A browser in the pane**, with back, forward, reload, and an address bar that takes a bare host — a real page in its own process, with no preload of its own. The agent can read through it (`browse_page`, `read_open_page`) but not drive it: fetching web content through a real browser is this surface's job, while automating a site stays Playwright's.
