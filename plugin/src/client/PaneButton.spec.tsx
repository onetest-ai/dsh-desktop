...
describe('PaneButton', () => {
  ...
  // loads it. In a plain browser there is no pane, so a button offering to
  ...
  it('toggles the pane when clicked', () => {
    const bridge = bridged()
    render(<PaneButton wide />)
    fireEvent.click(screen.getByRole('button', { name: 'Toggle the side pane' }))
  ...
  it('drops its label in the narrow rail, keeping the accessible name', () => {
    ...
    expect(screen.getByRole('button', { name: 'Toggle the side pane' })).not.toHaveTextContent('Side pane')
  })

  it('shows the label when the sidebar is wide', () => {
    bridged()
    render(<PaneButton wide />)
    expect(screen.getByRole('button', { name: 'Toggle the side pane' })).toHaveTextContent('Side pane')
  })
})
