...
**Cards are task-level workitems and bugs.** A mission is a lane label carrying its own status glyph; a campaign is a heading. Containers are structure here, and their statuses are read rather than dragged — which follows from the store's rule that a status is a claim with an author. A campaign and a task are not comparable units of work, and a board that put them in the same column would stop reading as a board.

**Tests are not cards.** They have no status, so there is no column they belong in, and a test is not work in flight — it is what the work is measured with. Instead a card carries a **verdict dot and a count**: a small dot coloured by the worst of that workitem's own `validated_by` verdicts — green when every one passed, red when any failed, neutral when none has run — followed by the plain count, `3/4`. The colour is what is worth seeing from across the board; the count is what you read once you're already looking. Clicking the dot opens the workitem's detail, where the links and their verdicts are.

The tests themselves are browsed in the tree and on the board's own Tests destination — see *Seeing tests*.

### What a card shows

Above its name, in a plainer mono line, the last segment of its own folder — the slug an agent or a terminal already knows this entity by, so the card and the file system name the same thing the same way. Then a status glyph beside the name itself, which wraps to two lines and then ellipses — a card that cannot show what it is called is not a card, which is the first thing the original columns got wrong by being too narrow to hold one. Beneath it, quieter: its type when it is a bug, how many acceptance criteria are ticked, and its verdict dot and count when anything validates it. Nothing else. A card is scanned, not read; what it is *for* lives in its detail, one click away.

**A status reads as a glyph and a label wherever the board draws it, not as a field name.** A small circle fills further as work moves idea → backlog → executing → validation → done, plain at idea and solid green only at done, so progress across the board is legible before a single word of it is read; the word beside it — `validation` rendering as Validation, in a column heading, on a mission's lane — is what names the glyph for anyone who cannot rely on colour alone. The stored value is untouched — the display is the view's business and the file's word is the store's — and one status shown twice on one screen should not read two ways.
...
**What it shows**, in octoshell's order, which is the order a reader wants: the name and its status — the same glyph and label the board draws, not a field of its own invention — as a heading, the parent it belongs to under that, then the description, then the level's own sections, then its children. Prose sections render as markdown — a test's Steps table is a table, not a wall of pipes.
