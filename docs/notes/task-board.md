...
**Opaque has one honest limit, and the writer pays it rather than the reader.** A body cannot hold a line that is itself a `##` heading, because in markdown that line *is* a new section, and a format cannot both split on `##` and promise that a `##` inside a body means something else. The choice is which side gives way. Making the reader split only on the headings it already knows would keep the byte, and would cost the `## Rollout` nobody modelled — it would be read as part of whatever section stood above it, which is the passthrough promised one paragraph down. So the writer gives way: a `## Design` written into a body is stored as `### Design`, which is a heading still, in the place it was written, nested under the section it actually sits in — what the writer would have typed had they known the enclosing heading was already a `##`. Nothing is escaped and no word is lost; one hash is added, once, and the file stops changing after the write that adds it. Two smaller rules come from the same requirement: a code fence a writer never closed is closed at the end of the body it was opened in, so that one stray backtick line cannot swallow the headings below it and make every later write re-emit them; and a body's blank edges are trimmed while its **columns are not**, because a four-space-indented code block is opaque markdown too and trimming it by character turned it into a paragraph followed by a stray line. The property all three exist to buy is worth naming: a document written and read back is the document that was written, field for field, and a file that is written twice with nothing changed is byte-identical the second time.

**A `##` that never gets its own heading, and a fence that closes in the wrong section.** Two things follow from the same reading, and neither is a bug to be fixed. The first is small: `##` with nothing but spaces after it is not a heading here, it is prose. A section is addressed by its name, a heading with no text has no name to be addressed by, and writing one back out produces a line the reader does not recognise as a heading — the round trip broken on a line nobody meant anything by. It stays where it was typed, verbatim.

The second is not small, and it is stated rather than closed. Fences are matched across the whole document, not within a section, so a fence a writer opened under `## Steps to Reproduce` and closed under `## Expected` swallows the `## Expected` between them: that field reads as nothing, its text renders as code inside the steps, and no write ever repairs it — though no write makes it worse either, and the file is byte-identical from the second write on. The obvious rule would close it: require a fence to close before the next column-zero `##`. We do not take it, because it would break the case this format is most right about. A body that holds a fenced ` ``` `/`## Design`/` ``` ` code sample is showing you a heading on purpose, and that sample must survive being written; a rule that ends fences at headings turns the sample's own heading back into a section and destroys exactly the content the demotion rule was built to protect. Between a fence that is genuinely closed being read as prose, and an unbalanced fence keeping its section, the closed fence wins — it is the case a person actually writes. The cost is one field reading empty, in a file whose fences do not balance the way the writer thought they did, and it is visible: the section is there, blank, with its text sitting in a code block above it. Balancing the fence in the editor fixes it, and nothing was lost in the meantime.

**Line endings are LF, and there are three kinds of them.** A file saved with `\r\n` — a Windows checkout, `core.autocrlf=true`, a `.gitattributes` that says so — is read correctly and written back with `\n`. Reading it correctly is not optional: the frontmatter fence is a line, and a reader that missed it took the entity's own `name`, `status` and `validated_by` as prose and re-emitted them inside the description on the next write, which destroyed them as data. Writing it back unchanged is not actually available, because the frontmatter fence and the YAML under it are emitted by this code and by js-yaml, both unconditionally LF — preserving CRLF in the bodies alone would write a file with two dialects in it. So one write converts a file's line endings, once, and it shows up in a diff the same way the demoted `##` does. **A bare `\r` counts as a line ending too**, which is one character of pattern and closes two holes rather than one. A classic-Mac file destroys itself the same way a CRLF file did — the dialect is dead, but the file is on disk and the reader is the thing that decides whether it survives — and a `\r` that merely *ends* a line was not harmless either: the writer appends the body's own `\n` after it, and the next read eats the pair, so a document that had settled still changed on being written. What it costs is a lone `\r` in the middle of a line, which is split into two lines rather than carried through; that is what CommonMark already says a bare `\r` means, and in every file that has one, it *is* where the line ended.
